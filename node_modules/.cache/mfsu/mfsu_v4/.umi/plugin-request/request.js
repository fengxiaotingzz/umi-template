import axios from "/Users/xiaotingfeng/demo/umi-demo/umi-demo2/node_modules/axios";
import useUmiRequest, { UseRequestProvider } from "/Users/xiaotingfeng/demo/umi-demo/umi-demo2/node_modules/@ahooksjs/use-request";
...
const getRequestInstance = () => {
  ...
  config2?.requestInterceptors?.forEach((interceptor) => {
    if (interceptor instanceof Array) {
      requestInstance.interceptors.request.use((config3) => {
        const { url } = config3;
        if (interceptor[0].length === 2) {
          const { url: newUrl, options } = interceptor[0](url, config3);
      ...
      requestInstance.interceptors.request.use((config3) => {
        ...
        if (interceptor.length === 2) {
          const { url: newUrl, options } = interceptor(url, config3);
          return { ...options, url: newUrl };
        }
...
const request = (url, opts = { method: "GET" }) => {
  ...
  const requestInterceptorsToEject = requestInterceptors?.map((interceptor) => {
    if (interceptor instanceof Array) {
      return requestInstance2.interceptors.request.use((config3) => {
        const { url: url2 } = config3;
        if (interceptor[0].length === 2) {
          const { url: newUrl, options } = interceptor[0](url2, config3);
          return { ...options, url: newUrl };
        }
        return interceptor[0](config3);
      }, interceptor[1]);
      ...
      return requestInstance2.interceptors.request.use((config3) => {
        const { url: url2 } = config3;
        if (interceptor.length === 2) {
          const { url: newUrl, options } = interceptor(url2, config3);
          return { ...options, url: newUrl };
        }
        return interceptor(config3);
      });
